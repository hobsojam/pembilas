...
|------|---------|---------|--------|
| `data/words.json` | 9,739 root words with English glosses | [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/) | Derived from [FreeDict eng-ind](https://freedict.org/) / [WikDict](http://www.wikdict.com/) / [Wiktionary](https://www.wiktionary.org/) |
| `data/affixes.json` | Affix rules and nasal assimilation patterns | Project authors | Hand-authored |
| `data/annotations.json` | Per-word affix applicability and derived meanings | Project authors | Hand-curated in 71 batches (issue #14, now closed) — **2,345 of 9,739 roots annotated** (6,109 annotation entries). Remaining roots fall back to the mechanical/unverified derivation path (see "Design decisions" above); this is the final coverage level, not a gap awaiting a future batch. |
...
- [x] Seed `annotations.json` with hand-curated root words
- [x] Deploy to GitHub Pages
- [x] Add `per-...-an` as a distinct noun circumfix (separate from `pe-...-an`)
- [x] Set up CI, Dependabot, and SonarCloud
- [x] Conclude the annotation effort (issue #14, closed) — 2,345 of 9,739 roots annotated across 71 batches

The annotation effort is done: further roots stay on the mechanical/unverified derivation path rather than being individually curated. See the [issue tracker](https://github.com/hobsojam/pembilas/issues) for remaining smaller items — accessibility fixes and a handful of known irregular-word edge cases the affix engine can't model yet.
